...
void DrawMGtestBQCDEWK(std::string var = "mWW", int NBIN = 1000, int MIN = 0, int MAX = 1000, std::string varHR = "mWW") {
 ...
 std::cout << " name EWK w/  b   = " << name1.Data() << std::endl;
 std::cout << " name EWK w/o b   = " << name2.Data() << std::endl;
...
//   gROOT->ProcessLine ("float xsecEWQCD[100] = {1.544};");
 ...
 gROOT->ProcessLine ("float xsecQCDwithoutB[100] = {0.1754};");
...
//  TString weightWithXsecEW    = Form ("(%s) * (%f) * (%f) * (numb==0)",cut.Data(),xsecQCD[0],1000./tot_1);
 ...
 leg->AddEntry(h_mWW_1,"qcd+ewk w/  b","l");
 leg->AddEntry(h_mWW_2,"qcd+ewk w/o b","l");
 leg->SetFillStyle(0);
...
//    h_Ratio -> SetBinContent (iBin+1, (ewk - qcd) / ewk);
 ...
 std::cout << std::endl;
 std::cout << " --------------------------- " << std::endl;
 std::cout << " qcd+ewk w/  b   = " << h_mWW_1->Integral() << std::endl;
 std::cout << " qcd+ewk w/o b   = " << h_mWW_2->Integral() << std::endl;
 std::cout << " --------------------------- " << std::endl;
 std::cout << std::endl; 
}