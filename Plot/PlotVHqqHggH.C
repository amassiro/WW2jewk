...
//             _labelOffset      = 0.02;
        ...
        void set_plotSigAlone (bool plotSigAlone) {
         _plotSigAlone = plotSigAlone;
        }
...
/*                     std::cout << "                  Y = " << Y + Y2 << " = " << Y << " + " << Y2 << std::endl; */
        ...
        //---- add weights S/(S+B) line-by-line ----
        
        void addWeight1D(int nCycle) {
...
//                 _vectTHstackSig.at(iSig) ->SetFillStyle (3001);//---> just for Ulrich and VBF
                _vectTHstackSig.at(iSig) ->SetFillColor ( _vectTHstackSig.at(iSig)->GetLineColor());//---> just for Ulrich and VBF
...
//                     std::cout << " iSig = " << iSig << " :: " << _vectTHSig.size() << std::endl;
            ...
            //---- prepare style for signal histograms (in case of drawing  over background
            for (unsigned int iSig = 0; iSig<_vectTHSig.size(); iSig++) {
// //                 _vectTHSig.at(iSig) ->SetFillStyle (3001);//---> just for Ulrich and VBF
                _vectTHSig.at(iSig) ->SetFillColor ( _vectTHSig.at(iSig)->GetLineColor());//---> just for Ulrich and VBF
                _vectTHSig.at(iSig) -> SetFillStyle(1001);
                _vectTHSig.at(iSig) -> SetLineColor(_vectColourSig.at(iSig));
                _vectTHSig.at(iSig) -> SetLineWidth(3);
            }
...
//           _vectTHSig.at(iSig) ->SetFillStyle (3001);//---> just for Ulrich and VBF
          _vectTHSig.at(iSig) ->SetFillColor ( _vectTHSig.at(iSig)->GetLineColor());//---> just for Ulrich and VBF
          _vectTHSig.at(iSig) -> SetFillStyle(0);
//           _vectTHSig.at(iSig) -> SetFillStyle(1001);
          _vectTHSig.at(iSig) -> SetLineColor(_vectColourSig.at(iSig));
          _vectTHSig.at(iSig) -> SetLineWidth(3);
         }
...
//             this->_legendTextSize *= (fontscale/1.2);
            ...
            TH1 *data   = GetDataHist();
            TGraphAsymmErrors *gr_data  = GetDataGR();
            ...
            for (unsigned int iSig = 0; iSig<_vectTHstackSig.size(); iSig++) {
                if (_mergeSignal == 0 ||  iSig == (_vectTHstackSig.size()-1)) {
                    ...
                    if (_plotSigAlone == true) {
                     if (iSig == 1) _vectTHstackSig.at(iSig) -> SetLineStyle(2); //---- the second "dashed"
                     _vectTHstackSig.at(iSig) -> Draw("hist,same");
                    }
                    else {
                     _vectTHstackSig.at(iSig) -> SetLineStyle(1);
                    }
                }
            }

//             if (data)     data->Draw("ep,same");
            if (gr_data)     gr_data->Draw("P");
...
//                         g_rdat->SetPointError (i, summed->GetBinWidth(i+1)/2.  , summed->GetBinWidth(i+1)/2.  , rdat->GetBinError(i+1)/scale, rdat->GetBinError(i+1)/scale);
                        // ---> new!
                        g_rdat->SetPointError (i, summed->GetBinWidth(i+1)/2.  , summed->GetBinWidth(i+1)/2.  , gr_data->GetErrorYlow(i) / scale,  gr_data->GetErrorYhigh(i) / scale);                        
...
//                 TPad *pad2 = new TPad("pad2","pad2",0,0,1,1-0.714609572);
                TPad *pad2 = new TPad("pad2","pad2",0,0,1,1-0.74);
...
//                 rref->GetYaxis()->SetRangeUser(0., 2.);
                ...
                float temp_titleOffset = _titleOffset;
                _titleOffset = 1.6;
...
//                 rref->GetYaxis()->SetTitle("data / sim");
                rref->GetYaxis()->SetTitle("data/exp");
            ...
            if (cAdditional) {
                ...
                for (int i = 0, n = summed->GetNbinsX(); i < n; ++i) {
                    ...
                    if (data) {
                        ...
                        //---> new!
                        errYloDataMinusBkg = sqrt(errYloDataMinusBkg_Syst*errYloDataMinusBkg_Syst + _gr_data->GetErrorYlow(i) * _gr_data->GetErrorYlow(i)); //---- poissonian
                        errYupDataMinusBkg = sqrt(errYupDataMinusBkg_Syst*errYupDataMinusBkg_Syst + _gr_data->GetErrorYhigh(i) * _gr_data->GetErrorYhigh(i)); //---- poissonian
...
//                         errYupDataMinusBkg = sqrt(errYupDataMinusBkg_Syst*errYupDataMinusBkg_Syst + data->GetBinContent(i+1)); //---- poissonian
                        ...
                        //---> new!
                        errYloDataMinusBkg_Stat = _gr_data->GetErrorYlow(i);
                        errYupDataMinusBkg_Stat = _gr_data->GetErrorYhigh(i);
//                         errYloDataMinusBkg_Stat = data->GetBinError(i+1);
//                         errYupDataMinusBkg_Stat = data->GetBinError(i+1);

                        //---> new! ---> already taken into account!
...
//                              temp_vectTHstackSig.at(iSig) ->GetYaxis () -> SetRangeUser(-10 ,45); //--- for time evolution
        ...
        ///---- TGraphAsymmErrors for data (correct asymm for low statistics)
        TGraphAsymmErrors* GetDataGR() { 
         ...
         if (_isWeighted) {
          ...
          for (int i = 0, n = _data->GetNbinsX(); i < n; ++i) {
           _gr_data->SetPoint      (i,_data->GetBinCenter(i+1), _data->GetBinContent(i+1));
           _gr_data->SetPointError (i,_data->GetBinWidth(i+1)/2. , _data->GetBinWidth(i+1)/2.  , _data->GetBinError(i+1),  _data->GetBinError(i+1));
          }
          
          _gr_data->SetLineWidth    (2);
          _gr_data->SetMarkerColor  (kBlack);
          _gr_data->SetLineColor  (kBlack);
          _gr_data->SetMarkerStyle(kFullCircle);
          
         }
         else if (!_gr_data && _data ) {
          _gr_data = new TGraphAsymmErrors();
          
          for (int i = 0, n = _data->GetNbinsX(); i < n; ++i) {
           _gr_data->SetPoint      (i,_data->GetBinCenter(i+1), _data->GetBinContent(i+1));
           _gr_data->SetPointError (i,_data->GetBinWidth(i+1)/2. , _data->GetBinWidth(i+1)/2.  , 0, 0);
           
           double N = _data->GetBinContent(i+1);
           double alpha=(1-0.6827);
           double L =  (N==0) ? 0  : (ROOT::Math::gamma_quantile(alpha/2,N,1.));
           double U =  (N==0) ?  ( ROOT::Math::gamma_quantile_c(alpha,N+1,1.) ) :
           ( ROOT::Math::gamma_quantile_c(alpha/2,N+1,1.) );
           _gr_data->SetPointEYlow(i,double(N)-L);
           if(N > 0) {
            _gr_data->SetPointEYhigh(i, U-double(N));
           }
           else { //---> fix 0 error in case of 0 events --> bayesian poisson 0 events observed  ---> my!
            _gr_data->SetPointEYhigh(i, 1.14);
           }
          }
...
//                     _vectTHstackSig.at(iSig) -> SetLineStyle(1);
                ...
                for (unsigned int iSig = 0; iSig<_vectTHSig.size(); iSig++) {
//                     _vectTHSig.at (iSig)->SetFillStyle(2001); // Ulrich for VBF only
                    _vectTHSig.at(iSig) ->SetFillColor ( _vectTHSig.at(iSig)->GetLineColor());//---> just for Ulrich and VBF
...
//             float scaleBy = 1.35 + 0.2*(sampCount>6) + 0.2*(sampCount>10) + 0.2*(sampCount>14);
            float scaleBy = 1.75 + 0.2*(sampCount>6) + 0.2*(sampCount>10) + 0.2*(sampCount>14);
...
//                  THStackAxisFonts(hstack, "y", TString::Format("entries / %s",_units.Data()));
                 THStackAxisFonts(hstack, "y", TString::Format("Events / %s",_units.Data()));
                }
...
//                         _vectTHstackSig.at(iSig) ->SetFillStyle (3001);//---> just for Ulrich and VBF
                        DrawLegend(x0+pos[1]*wx, _globalYoffset - off[1]*_yoffset, _vectTHstackSig.at(iSig)         , name4Legend.Data() ,           "f" );
                    }
                    else {
//                         DrawLegend(x0+pos[j]*wx, _globalYoffset - off[j]*_yoffset, _vectTHstackSig.at(iSig)         , _vectNameSig.at(iSig) ,           "l" );
//                         _vectTHstackSig.at(iSig) -> SetFillStyle (3001);//---> just for Ulrich and VBF
                        _vectTHstackSig.at(iSig) ->SetFillColor ( _vectTHstackSig.at(iSig)->GetLineColor());//---> just for Ulrich and VBF
                        DrawLegend(x0+pos[j]*wx, _globalYoffset - off[j]*_yoffset, _vectTHstackSig.at(iSig)         , _vectNameSig.at(iSig) ,           "f" );
                        //---> just for Ulrich and VBF
                    }
                }
                j++;
            }
                ...
                TString option)
        {
            ...
            TH1* cloneHist = (TH1*) hist->Clone();
            std::cout << " cloneHist->GetFillStyle[" << cloneHist->GetTitle () << "] = " << cloneHist->GetFillStyle () << " option.Data() = " << option.Data() << std::endl;
//             cloneHist->SetFillStyle (3001);
            legend->AddEntry(cloneHist, label.Data(), option.Data());
...
/*             TAxis* ay = xxx->GetYaxis(); */
        ...
        bool _plotSigAlone; //---- to plot signal not-stacked to the background also

        bool _isWeighted; //---- to have errors on data with quantiles! If weighted, switch to normal error
